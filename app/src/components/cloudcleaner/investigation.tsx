        ...
        <div className="px-4 py-3" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          {(rec.action === "stop" || rec.action === "retire") && (
            <div className="mt-2 pt-2" style={{ borderTop: "1px dashed var(--border)" }}>
              <Row label="Current monthly cost" value={money(r.estimated_monthly_cost)} />
              <Row
                label={rec.action === "stop" ? "Cost after stop" : "Cost after retirement"}
                value={money(costAfterAction)}
              />
              <Row
                label="Estimated monthly saving"
                value={money(recommendedSaving)}
                tone="var(--ok)"
              />
            </div>
          )}
          ...
          <p id="approve-help" className="mt-1.5 text-[11px]" style={{ color: "var(--fg-faint)" }}>
            {command && !matches
              ...
              : plan?.steps.length
                ...
                : "Stopping is reversible; policy requires approval before execution."}
          </p>
        </form>
      )}
